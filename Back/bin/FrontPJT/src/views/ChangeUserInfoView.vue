<script setup>
import { changeUserInfo, deleteUser } from "@/api/member";
...
function deleteuser() {
    ...
    deleteUser(body, ({ data }) => {
        ...
        sessionStorage.removeItem("acceess_token");
...
<template>
    <div>
        ...
        <span v-show="isMailConfirm">인증 번호
            <input :disabled="isConfirmed" v-model="inputString">
        ...
        <br>
        <div>
            비밀번호 입력<input v-model="password" id="password"/><br>
        </div>
